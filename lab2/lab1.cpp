...
void vector_add(float *result, const float *a, const float *b, int size) {
    sv_float4 passthru = sv_set_float(10.0f, 20.0f, 30.0f, 40.0f); // passthru vector
    int i = 0;

    for (; i < size; i += 4) {
        int remaining = size - i; // 計算需要處理的剩餘元素數量
        sv_mask tail_mask = sv_init_ones(remaining); // 生成遮罩
        // 讀值
        sv_float4 va = sv_load_float(passthru, a + i, tail_mask);
        sv_float4 vb = sv_load_float(passthru, b + i, tail_mask);
        
        // add 運算
        sv_float4 vr = sv_float_add(va, vb, tail_mask);

		// stoore resule
        sv_store_float(result + i, vr, tail_mask);
    }
}
